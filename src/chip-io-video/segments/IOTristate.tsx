...
export const IOTristate: React.FC<{ spec?: TristateSpec }> = ({ spec: customSpec }) => {
  ...
  const oeColor = phase.oe ? THEME.status.hit : THEME.text.muted;
  const wireColor = isHighZ ? "#475569" : (pmosOn ? "#f472b6" : "#60a5fa");

  // Status text
  const conductingText = isHighZ ? "无（高阻）" : (pmosOn ? "PMOS" : "NMOS");
  const padValueText = isHighZ ? "Z（高阻）" : String(phase.padOutput);
      ...
      <svg viewBox={`0 0 ${VW} ${VH}`} width="100%" height="100%">
        ...
        <g>
          ...
          <text x={240} y={80} textAnchor="middle"
            fill={phase.oe ? THEME.status.hit : THEME.status.miss}
          ...
          <text x={385} y={80} textAnchor="middle"
            fill={phase.data === "Z" ? THEME.text.muted : (phase.data ? THEME.status.hit : THEME.status.miss)}
            fontSize={18} fontWeight={700} fontFamily="Inter, sans-serif">
          ...
          <text x={770} y={80} textAnchor="middle"
            fill={isHighZ ? "#94a3b8" : (phase.padOutput === 1 ? THEME.status.hit : THEME.status.miss)}
            fontSize={18} fontWeight={700} fontFamily="Inter, sans-serif">
            {padValueText}
          </text>
        ...
        <g transform="translate(80, 200)">
          ...
          <rect x={0} y={10} width={80} height={30} rx={4}
            fill={phase.oe ? THEME.status.hit : "#475569"} fillOpacity={0.3}
            stroke={phase.oe ? THEME.status.hit : "#475569"} strokeWidth={1.5} />
          <text x={40} y={30} textAnchor="middle" fill={phase.oe ? THEME.status.hit : THEME.text.muted}
        ...
        <g transform="translate(80, 280)">
          ...
          <rect x={0} y={10} width={80} height={30} rx={4}
            fill={phase.data === "Z" ? "#475569" : (phase.data ? THEME.status.hit : THEME.status.miss)} fillOpacity={0.3}
            stroke={phase.data === "Z" ? "#475569" : (phase.data ? THEME.status.hit : THEME.status.miss)} strokeWidth={1.5} />
